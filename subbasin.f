...
!! Hailiang for debug purpose ~ start ~
      integer :: jj
      jj = hrutot(inum1)
!! Hailiang for debug purpose ~ end ~
      ihru = 0
      ihru = hru1(inum1)
...
!       if (ipot(j) /= j .and. imp_trig(nro(j),nrelease(j),j)==1)       &
     ...
     &        then             
        ...
        !! snow redistribution Hailiang April 14 2011 ~~ end ~~ 
          ...
          if (irrsc(j) > 2) then
            ...
            if (iir(nro(j),nirr(j),j) > 0) then
              ...
              if (igro(j) == 0) then
                ...
                aphu = phubase(j)
                phuop = phuirr_nocrop(nro(j),nirr(j),j)
              else
                aphu = phuacc(j) 
                phuop = phuirr(nro(j),nirr(j),j)
              end if
              if (phuop > 0. .and. aphu > phuop) then
                call irrsub
	          if (imgt == 1) then
