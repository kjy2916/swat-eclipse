...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~      
      ...
      total_area = sum(hru_km)
      
      !! calculate WR, change the number of hru to total_area/area(i)
      sr_wr = sr_w(ihru) * total_area / hru_km(ihru) / sum(sr_w)
      
      !! calculate SAP, use sno_hru2
      sr_sap = sr_wr * sum(sno_hru2 * hru_km) / total_area
