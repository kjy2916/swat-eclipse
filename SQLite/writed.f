...
!!    writes out the amount of water stored in the soil layer
      if (isto > 0) then 
        do j = 1, nhru
            !!~ ~ ~ SQLite ~ ~ ~
            if(ioutput == 1) then
                call sqlite3_set_column( colswr(1), iyr )
                call sqlite3_set_column( colswr(2), i_mo )
                call sqlite3_set_column( colswr(3), icl(iida) )
                do j1 = 1, 10
                    if(j1 > sol_nly(j)) then
                      call sqlite3_set_column(colswr(3+j1),0.0)
                    else
                      call sqlite3_set_column(colswr(3+j1),sol_st(j1,j))
                    end if
                end do
                call sqlite3_insert( db, tblswr(j), colswr )
            else
