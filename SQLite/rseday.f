...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      do j = 1, subtot
          ...
          if(ioutput == 1) then
            ...
            call sqlite3_set_column( colsed(2), i_mo )
            call sqlite3_set_column( colsed(3), icl(iida) )
            call sqlite3_set_column( colsed(4), rchdy(5,j) )
            call sqlite3_set_column( colsed(5), rchdy(6,j) )
            do ii = 43,59
                call sqlite3_set_column( colsed(ii-37), rchdy(ii,j) )
            end do
