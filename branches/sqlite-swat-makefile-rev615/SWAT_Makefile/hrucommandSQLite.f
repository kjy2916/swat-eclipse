      subroutine hrucommandSQLite(hru_id,period,all_result,part_result)

      use parm

      integer, intent(in) :: hru_id
      real, intent(in), dimension(mhruo) :: all_result
      real, intent(in), dimension(mhruo) :: part_result
      ...
      !!Zhiqiang, 2012-9-25
       ...
       !!subbasin number
       write(sqlitedata,"(i10)") hru_sub(j)
       sqlcmd = sqlcmd(:len_trim(sqlcmd)) // ',' // sqlitedata
