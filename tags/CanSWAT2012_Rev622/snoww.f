...
!!    total w for each subbasin,zhiqiang,2014-2-28
      real, dimension (msub) :: sub_w
...
!!    calculate W, change daily
      ...
      !!Zhiqiang, 2014-2-28
      !!calculate total w for each subbasin
      sub_w = 0
      do ii = 1, nhru
        sub_w(hru_sub(ii)) = sub_w(hru_sub(ii)) + sr_w(ii)
      end do
      !!calculate w/sub_w for each hru, which is used in wr (snowrd.f)
      do ii = 1, nhru
        if(sub_w(hru_sub(ii)) > 1.e-6) then
            sr_w(ii) = sr_w(ii)/sub_w(hru_sub(ii))
        else
            sr_w(ii) = 1.
        end if
      end do
